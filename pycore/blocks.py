...
def block_Unconv(name, bottom, top, s_filer=256, n_filer=64, offset="(1,0,0)", size=(32,32,3.5), opacity=0.5):
    return [
        ...
        to_Conv(    name='ccr_{}'.format(name),       offset="(0,0,0)", to="(ccr_res_{}-east)".format(name),   spatial_size=str(s_filer), filter_num=str(n_filer), width=size[2], height=size[0], depth=size[1]),
        ...
        to_Conv(    name='{}'.format(top),            offset="(0,0,0)", to="(ccr_res_c_{}-east)".format(name), spatial_size=str(s_filer), filter_num=str(n_filer), width=size[2], height=size[0], depth=size[1]),
        to_connection( 
            "{}".format(bottom), 
            "unpool_{}".format(name) 
            )
...
def block_Res(num, name, bottom, top, s_filer=256, n_filer=64, offset="(0,0,0)", size=(32,32,3.5), opacity=0.5):
    ...
    for name in layers:        
        ly = [to_Conv( 
            ...
            offset=offset, 
            to="({}-east)".format(bottom),   
            spatial_size=str(s_filer), 
            filter_num=str(n_filer), 
            width=size[2],
            height=size[0],
            depth=size[1]
            ),
